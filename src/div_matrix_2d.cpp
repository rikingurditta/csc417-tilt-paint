...
void div_matrix_2d(const int nx, const int ny, Eigen::SparseMatrix<double> B) {
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            int cell = i + j * nx;
            tl.emplace_back(cell, i + j * (nx + 1), -1);
            tl.emplace_back(cell, i + j * (nx + 1) + 1, 1);
            tl.emplace_back(cell, i + j * nx + dx_grid_size, -1);
            tl.emplace_back(cell, i + (j + 1) * nx + dx_grid_size, 1);
        }
    }
